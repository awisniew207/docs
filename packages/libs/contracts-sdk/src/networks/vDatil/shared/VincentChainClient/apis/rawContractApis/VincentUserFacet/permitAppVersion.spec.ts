...
import { getAllPermittedAppIdsForPkp, getPermittedAppVersionForPkp } from '../VincentUserViewFacet';
...
describe('permitAppVersion', () => {
  ...
  it('should permit an app version for a PKP token', async () => {
    ...
    let initialPkps;
    try {
      initialPkps = await getAllRegisteredAgentPkps({ userAddress }, testContext.networkContext);
