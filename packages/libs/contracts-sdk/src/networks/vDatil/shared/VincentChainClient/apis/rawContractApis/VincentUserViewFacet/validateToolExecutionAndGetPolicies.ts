...
const ExpectedParams = z
  ...
  .transform(
    (params): RawContractParams => [params.delegatee, params.pkpTokenId, params.toolIpfsCid],
  );
...
export async function validateToolExecutionAndGetPolicies(
  ...
  const validation =
    await vincentUserViewFacetContract.read.validateToolExecutionAndGetPolicies(validatedRequest);
