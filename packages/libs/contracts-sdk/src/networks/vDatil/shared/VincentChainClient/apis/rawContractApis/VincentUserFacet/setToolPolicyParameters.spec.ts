...
describe('setToolPolicyParameters', () => {
  ...
  it('should set tool policy parameters for a PKP token', async () => {
    ...
    if (parameterSetEvent) {
      expect(parameterSetEvent.args.pkpTokenId).toBe(testContext.AGENT_PKP_TOKEN_IDS[0]);
      expect(parameterSetEvent.args.appId).toBe(testContext.registerAppRes.appId);
    ...
    expect(toolsAndPolicies[0].policies[0].parameters.length).toBeGreaterThan(0);
  });
});
