...
export const getTestContext = async (opts?: { registerApp?: boolean; jit?: boolean }) => {
  ...
  if (opts?.registerApp) {
    ...
    appId = registerAppRes.decodedLogs.find((log) => log.eventName === 'NewAppVersionRegistered')
      ?.args.appId;
    managerAddress = registerAppRes.decodedLogs.find((log) => log.eventName === 'NewAppRegistered')
      ?.args.manager;
