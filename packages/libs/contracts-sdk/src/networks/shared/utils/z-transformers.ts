...
// eg. "2" or 2 -> 2n
export const toBigInt = z.union([z.string(), z.number()]).transform((n) => safeBigInt(n));
...
export const toBigIntMatrix = z
  .union([z.array(z.array(z.union([z.string(), z.number(), z.bigint()]))), z.undefined()])
  .transform((val) => {
    if (!val) return [[]];
    return val.map((inner) => inner.map((v) => (typeof v === 'bigint' ? v : safeBigInt(v))));
  });
...
// eg. "2" -> 2
// eg. 2n -> 2
export const toNumber = z.union([z.bigint(), z.number(), z.string()]).transform((val) => {
  return Number(val);
});
