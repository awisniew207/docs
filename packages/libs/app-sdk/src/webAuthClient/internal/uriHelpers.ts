import { verifyVincentAppUserJWT } from '../../jwt/core/verify';
import { JWT_URL_KEY, PRODUCTION_VINCENT_DASHBOARD_URL } from '../constants';

export const decodeVincentJWTFromUri = async ({
  uri,
  expectedAudience,
  requiredAppId,
}: {
  ...
  try {
    return {
      decodedJWT: await verifyVincentAppUserJWT({ jwt, expectedAudience, requiredAppId }),
      jwtStr: jwt,
    };
  } catch (error) {
