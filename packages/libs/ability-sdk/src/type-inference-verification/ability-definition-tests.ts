...
export function testPolicyEvaluationResults() {
  ...
  const simplePolicy = createVincentAbilityPolicy({
  ...
  return createVincentAbility({
    ...
    abilityParamsSchema: baseAbilitySchema,
    supportedPolicies: supportedPoliciesForAbility([simplePolicy, commitPolicy]),
    ...
    precheck: async ({ abilityParams }, { fail, succeed }) => {
      ...
      if (!abilityParams.action || !abilityParams.target || abilityParams.amount <= 0) {
        ...
        if (!abilityParams.action) issues.push('Missing action');
        if (!abilityParams.target) issues.push('Missing target');
        if (abilityParams.amount <= 0) issues.push('Amount must be positive');
