...
describe('unPermitAppVersion', () => {
  ...
  it('should unpermit an app version for a PKP token', async () => {
    ...
    if (appVersionUnPermittedEvent) {
      expect(appVersionUnPermittedEvent.args.pkpTokenId).toBe(
        testContext.AGENT_PKP_TOKEN_IDS[0],
      );
      expect(appVersionUnPermittedEvent.args.appId).toBe(
        testContext.registerAppRes.appId,
      );
      expect(appVersionUnPermittedEvent.args.appVersion).toBe(
        appVersionRes[1].version,
      );
    }
