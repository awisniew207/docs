...
const getMainMenuItems = (apps: App[], isLoadingApps: boolean, permittedAppVersions: Record<string, string> = {}, appVersionsMap: Record<string, any[]> = {}): MenuItem[] => {
  return [
  {
    id: 'apps',
    label: 'Apps',
    ...
    children: isLoadingApps
      ...
      : apps.length === 0
        ...
        : apps.map((app) => {
            ...
            const hasVersionMismatch = !!(permittedVersion && activeVersion && permittedVersion !== activeVersion);
...
export function AppSidebar({ apps = [], permittedAppVersions = {}, appVersionsMap = {}, isLoadingApps = false }: AppSidebarProps) {
                                ...
                                <div className="relative group/triangle">
                                  <TriangleAlert 
                                    className={`h-4 w-4 transition-colors cursor-pointer ${
                                      child.warningType === 'yellow' ? 'text-yellow-500 hover:text-yellow-400' :
                                      child.warningType === 'orange' ? 'text-orange-500 hover:text-orange-400' :
                                      child.warningType === 'red' ? 'text-red-500 hover:text-red-400' :
                                  ...
                                  <div className="absolute left-full ml-2 top-1/2 transform -translate-y-1/2 opacity-0 group-hover/triangle:opacity-100 transition-opacity duration-200 pointer-events-none z-50">
                                    <div className="bg-gray-900 text-white text-[10px] rounded px-1.5 py-0.5 whitespace-nowrap shadow-lg">
                                      {child.warningType === 'yellow' && (
                                        <>
                                          ...
                                          <div className="text-gray-300">
                                            Permitted: v{permittedAppVersions[child.id]} | Active: v{apps.find(app => app.appId.toString() === child.id)?.activeVersion}
                                          </div>
                                        </>
                                      )}
                                      {child.warningType === 'orange' && (
                                        <>
                                          <div className="font-medium">
                                            {child.permittedVersionEnabled === false && child.activeVersionEnabled === false
                                      ...
                                      {child.warningType === 'red' && (
                                        <>
                                          <div className="font-medium">Permitted + Active Versions Disabled</div>
                                          <div className="text-gray-300">
                                            Permitted v{permittedAppVersions[child.id]} and Active v{apps.find(app => app.appId.toString() === child.id)?.activeVersion} are disabled
                                          </div>
                                        </>
                                      )}
                                    </div>
            ...
            <SidebarMenuItem>
              ...
              >
                <Link to="/user/wallet" className="flex items-center gap-3">
                  <div className={`${isActiveRoute('/user/wallet') ? 'text-orange-500' : themeStyles.textMuted}`}>
                    <Wallet className="h-4 w-4" />
                  </div>
                  <span className={`font-medium ${isActiveRoute('/user/wallet') ? 'text-orange-500' : themeStyles.text}`}>
                    Wallet
                  </span>
                </Link>
              </SidebarMenuButton>
