...
async function deployLitAction({ generatedDir, outputFile = 'lit-action.js', pinataJwt, type }) {
  try {
    ...
    if (!fs.existsSync(filePath)) {
      throw new Error(
        `Bundled Lit Action code string not found at ${filePath}. Please build the ability first.`,
      );
    }
    ...
    console.log(
      `✅ ${type === 'ability' ? '🛠️' : '⚖️'}${filePathArr.slice(filePathArr.length - 2, filePathArr.length - 1)} Deployed to IPFS`,
    );

    return ipfsCid;
  } catch (error) {
    console.error(
      "❌ ${type === 'ability' ? '🛠️' : '⚖️'}${filePathArr.slice(filePathArr.length - 2, filePathArr.length - 1)} Error deploying to IPFS:",
      error.message,
    );
    throw error;
  }
