...
const { buildVincentAbility, buildVincentPolicy } = require('./esbuild');

// Base directories for abilities and policies
...
/**
 * Gets all ability directories in the specified base directory and its subdirectories
...
/**
 * Builds all abilities and policies in the specified directories
 */
async function buildAllActions() {
  ...
  // Ensure IPFS CIDs embedded into the abilities are accurate by building all the policies first.
  const abilitiesResults = await Promise.all(
    abilities.map(async (ability) => {
      const { name: actionDir, entryPoint, outdir } = ability;

      try {
        await buildVincentAbility({
          entryPoint,
          outdir,
          tsconfigPath: TSCONFIG_PATH,
        });
  ...
  const results = [...policiesResults, ...abilitiesResults];
  // Print summary
