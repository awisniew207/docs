    ...
    for (const parameter of parameters) {
        switch (parameter.name) {
            ...
            case 'maxSpendingLimit':
                ...
                if (!durationParam) {
                    throw new Error('spendingLimitDuration not found in policy parameters, but required when maxSpendingLimit is set');
                }

                if (durationParam.paramType !== 2) {
                    throw new Error(`Unexpected parameter type for spendingLimitDuration: ${durationParam.paramType}`);
                }
