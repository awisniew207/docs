...
<Steps>
  <Step title="Precheck Phase">
    ...
    - Decodes the UserOperation calldata to understand the transactions
...
- **Valid UserOperation**: The backend service must construct a valid UserOperation containing Aave interactions
...
└────────┬────────┘
         ...
         ▼
┌─────────────────┐
│ Smart Account   │ Grants session key to
└────────┬────────┘
         │
         ▼
┌─────────────────┐
│  Agent Wallet   │ Signs via
└────────┬────────┘
         │
         ▼
┌─────────────────┐
│ This Ability    │ Validates & Signs UserOp
└─────────────────┘
```
...
<Tabs>
  <Tab title="Parameters">
    ...
    ```typescript
    {
      ...
      /**
       * RPC URL for on-chain simulation and validation
       */
      rpcUrl: string;
    }
    ```
    ...
    **Important Notes:**
    ...
    - **RPC URL**: Should be an Alchemy RPC URL for simulation support (`alchemy_simulateUserOperationAssetChanges`)
...
<Note>
Additional networks may work if your Smart Account provider supports them and Aave V3 is deployed there, but the networks listed above have verified ERC-4337 infrastructure support.
</Note>
